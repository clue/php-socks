...
class Socks5 extends Socks4
{
     ...
     *            port to connect to
     * @param int $method
     *            SOCKS method to use (connect/bind)
     * @return Stream
    ...
    protected function transceive($hostname, $port, $method)
    {
        ...
        if ($this->auth === NULL) {
            ...
            $packet .= pack('C2', 0x01, 0x00);
        } else {
            // two methods, username/password and no authentication
            $packet .= pack('C3', 0x02, 0x02, 0x00);
        }
        ...
        if ($data['method'] === 0x02 && $this->auth !== NULL) {
